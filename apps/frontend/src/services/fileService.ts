...
export const downloadZip = async (projectId: string, fileId: string): Promise<Response> => {
  const response = await fetch(`${API_BASE}/projects/${projectId}/files/${fileId}/download`, {
    method: 'GET',
    headers: {
...
export const uploadFiles = async (projectId: string, files: FormData): Promise<GetFilesRes> => {
  const response = await fetch(`${API_BASE}/projects/${projectId}/files`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${localStorage.getItem('token')}`,
